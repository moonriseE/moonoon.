...
from discord import ButtonStyle, SelectOption, Interaction, ButtonStyle
...
cluster = MongoClient(f"mongodb+srv://miskimka:emssotka@programma.iubdewp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster")
usersdb = cluster.WN.members

class PaginatedUserSelectView(View):
    def __init__(self, members, role_id, page=0):
    ...
    def update_select_menu(self):
        ...
        async def select_callback(interaction: Interaction):
            ...
            await interaction.response.send_message(f"{member.mention} был успешно уволен!", ephemeral=True)
            with open("channels.json") as file:
                data = json.load(file)
            if str(interaction.guild.id) in data:
                if "dismiss-redaction" in data[str(interaction.guild.id)]:
                    channel = interaction.guild.get_channel(
                        data[str(interaction.guild.id)]["dismiss-redaction"]
                    )
                    embed = discord.Embed(title="Увольнение сотрудника", color = discord.Color.red())
                    embed.add_field(name="Сотрудник", value=f"{member.mention} | {member}")
                    embed.add_field(name="Уволил", value=f"{interaction.user.mention} | {interaction.user}")
        ...
        if self.page > 0:
            prev_button = Button(label="⬅️ Назад", style=ButtonStyle.primary)
        ...
        if end < len(self.members):
            next_button = Button(label="➡️ Вперед", style=ButtonStyle.primary)
...
class AddWork(discord.ui.Modal):
    def __init__(self, message, *args, **kwargs):
        super().__init__(title="Допольнительная работа", *args, **kwargs)
        self.dop_work = discord.ui.TextInput(label="Доп. работа", placeholder="Своя работа в свободной форме. После отправки вы не сможете это редактировать!")
        self.message = message
        self.add_item(self.dop_work)

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer()
        embed = self.message.embeds[0]
        embed.add_field(name="Доп. работа:", value=self.dop_work)
        button = discord.ui.Button(
            label="Доп. работа",
            style=discord.ButtonStyle.green,
            disabled=True,
            custom_id=f"dop_work_{interaction.user.id}",
        )
...
class RejectReasonModal(discord.ui.Modal, title="Причина отклонения"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            f"{interaction.user.mention} отказал по причине: ```{self.reason.value}```"
        )

class Form_for_comment(discord.ui.Modal):
    def __init__(self, member:discord.Member, message, *args, **kwargs):
        super().__init__(title="Комментарий", *args, **kwargs)
        self.comment = discord.ui.TextInput(label="Комментарий отказа")
        self.member = member
        self.message = message
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer()
        await self.message.reply(f"{interaction.user.mention} отказал в повышении по причине: ```{self.comment}```")
        return


class Accept_Redaction(discord.ui.Modal):
    def __init__(self, bot:commands.Bot, member:discord.Member, *args, **kwargs):
        super().__init__(title="Кадровый аудит", *args, **kwargs)
        self.name = discord.ui.TextInput(
            label="Имя Фамилия | Паспорт",
            placeholder="И.Ф, паспорт строго через | вместе с пробелами"
        )
        self.bot = bot
        self.reason = discord.ui.TextInput(label="Причина принятия / увольнения")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed_error = discord.Embed(title='Произошла ошибка', color=discord.Color.from_rgb(255, 0, 0))
            embed_error.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar.url)
            if len(self.name.value.split(" | ")) != 2:
                embed_error.description = '**Должна соблюдаться строгая форма имени и паспорта:\n "Имя Фамилия | Паспорт"\nПример:**'
                embed_error.add_field(name='Имя фамилия | Паспорт', value='Maksim Postov | 321343', inline=False)
                await interaction.followup.send(embed=embed_error, ephemeral=True)
                return
            elif not self.name.value.split(" | ")[1].isdigit():
                embed_error.description = '**Вы неверно указали паспорт**'
                embed_error.add_field(name='Правильно✅', value='**Maksim Postov | 321343**', inline=False)
                embed_error.add_field(name='Неправильно❌', value='Указывать пробел или иной символ в конце паспорта, совмещать паспорт и буквы (321s343)')
                await interaction.followup.send(embed=embed_error, ephemeral=True)
                return
            embed = discord.Embed(
                title=f"Заявление на принятие в редакцию",
                color=0xDC143C,
            )
            embed.add_field(name="Тема:", value="Принять в редакцию", inline=False)
            embed.add_field(
                name="Сотрудник",
            ...
            embed.add_field(
                name="Дата",
                value=datetime.now(MSK).strftime("%d.%m.%Y"),
                inline=False,
            )
            embed.set_author(name=interaction.user, icon_url=interaction.user.avatar.url if interaction.user.avatar != None else interaction.user.default_avatar.url)
            message = await channel.send(embed=embed)
            db = {
                "_id": int(self.member_id),
                "Имя": self.name.value.split(" | ")[0],
                "Паспорт": int(self.name.value.split(" | ")[1])
            }
            test = usersdb.insert_one(db)
            print(db, test)
            await interaction.followup.send(
                content=f"Ваша заявка была успешно отправлена!\n{message.jump_url}",
                ephemeral=True,
            )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            async for _ in channel.history(limit=None):
                message_count += 1
            embed = discord.Embed(title=f"Заявление на повышение №{message_count}", color=0xDC143C)
            embed.add_field(
            ...
            embed.set_footer(text=datetime.now(MSK).strftime("%d.%m.%Y %H:%M:%S"), icon_url="https://images-ext-1.discordapp.net/external/UG_tfMhqNXSmvUYsE_rj_bupA2hUsZuekayH44Aca3M/https/i.imgur.com/UgIB5Al.png")
            embed.set_author(name=interaction.user, icon_url=interaction.user.avatar.url if interaction.user.avatar != None else interaction.user.default_avatar.url)
            view = View()
            view.add_item(
                Button(
                    ...
                    label="Принять",
                    custom_id=f"accept-promote-mod-yes:{str(interaction.user.id)}",
                    row=0,
                )
            )
            view.add_item(
                Button(
                    ...
                    label="Отклонить",
                    custom_id=f"accept-promote-mod-no:{str(interaction.user.id)}",
                    row=0,
                )
            )
...
class Recovery(discord.ui.Modal):
    def __init__(self, *args, **kwargs):
        super().__init__(title="Восстановление", *args, **kwargs)
        self.name = discord.ui.TextInput(label="Имя Фамилия:")
        self.pasport = discord.ui.TextInput(label="Паспорт:")
        self.job = discord.ui.TextInput(
            label="Должность, которую занимал сотрудник:"
        )
        self.contact = discord.ui.TextInput(label="Восстановлен на должность:")

        self.add_item(self.name)
        self.add_item(self.pasport)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            with open("channels.json") as file:
                data = json.load(file)
            if str(interaction.guild.id) in data:
                if "recovery" in data[str(interaction.guild.id)]:
                    channel = interaction.guild.get_channel(
                        data[str(interaction.guild.id)]["recovery"]
                    )
                else:
                    return await interaction.followup.send(
                        content="Ваша заявка не была отправлена, так как Владельцы Сервера не настроили канал для приёма заявок!",
            ...
            async for _ in channel.history(limit=None):
                message_count += 1
            embed = discord.Embed(
                title=f"Заявление на Восстановление №{message_count}", color=0xDC143C
            )
            embed.add_field(name="Тема:", value="Восстановление", inline=False)
            embed.add_field(
                name=self.name.label,
                value=self.name.value,
                inline=False,
            ...
            embed.add_field(
                name=self.pasport.label,
                value=self.pasport.value,
                inline=False,
            )
            embed.add_field(
                name=self.job.label,
            ...
            embed.add_field(
                name=self.contact.label,
                value=self.contact.value,
                inline=False,
            )
            embed.set_footer(text=datetime.now(MSK).strftime("%d.%m.%Y %H:%M:%S"), icon_url="https://images-ext-1.discordapp.net/external/UG_tfMhqNXSmvUYsE_rj_bupA2hUsZuekayH44Aca3M/https/i.imgur.com/UgIB5Al.png")
            embed.set_author(name=interaction.user, icon_url=interaction.user.avatar.url if interaction.user.avatar != None else interaction.user.default_avatar.url)
            await channel.send("<@&653928672381632532>", embed=embed)
            await interaction.followup.send(
                content="Ваша заявка была успешно отправлена!",
                ephemeral=True,
            )
...
class Human_Resources_Department(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                title=f"Заявление на повышение", color=0xDC143C
            )
            embed.add_field(
        ...
        try:
            ...
            view.add_item(
                Button(
                    style=ButtonStyle.green,
                    label="Принять",
                    custom_id=f"accept-quit-mod:{str(interaction.user.id)}",
                    row=0,
                )
            )
            view.add_item(
                Button(
                    style=ButtonStyle.red,
                    label="Отклонить",
                    custom_id=f"accept-quit-mod-no:{str(interaction.user.id)}",
                    row=0,
                )
            )
            await channel.send("<@&653928672381632532>", embed=embed, view=view)
            await interaction.followup.send(
                content="Ваша заявка была успешно отправлена!",
                ephemeral=True,
            )
...
class UserSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for selected_user in self.values:
            ...
            # elif self.task == "dismiss-redaction":
            #     member = interaction.guild.get_member(int(selected_user.id))
            #     role = discord.utils.get(interaction.guild.roles, id=1045788643287117854)
            #     await member.edit(roles=[])
            #     await member.add_roles(role)
            #     await interaction.response.send_message(f"{member.mention} был успешно уволен!", ephemeral=True, delete_after=60)
